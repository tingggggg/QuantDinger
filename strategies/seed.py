...
ITEMS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "「均價的百分比」，無法表達會移動的絕對價位。"
            "預設值來自 72 組參數掃描、依勝率排序（BTC 800 天、每筆風險 1%、"
            "低於 15 筆的組合排除）：HTF=4h／LTF=1h／trigger=1／reward_r=1.0／trail=1，"
            "全期勝率 60.7%、28 筆、PF 1.50。"
            "但請注意兩件事：一、依勝率選會把 reward_r 壓到最小（1.0 勝率最高但 PF 最差 1.17，"
            "2.0 的 PF 1.32 較好但勝率較低）；二、這組沒撐過樣本分割 —— "
            "前半勝率 70.6%／PF 2.21，後半 45.5%／PF 0.94。"
            "它是 72 組裡在單一樣本上最好的一組，是對這段區間的描述，不是優勢的證據。"
